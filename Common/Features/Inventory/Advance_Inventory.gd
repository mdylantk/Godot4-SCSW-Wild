...
class_name Advance_Inventory extends RefCounted

signal changed(slot:int)
signal max_size_changed(new_size:int)
...
func add_item(new_item:Item)->void:
	...
	if remaining_amount > 0:
		...
		for new_slot in range(max_size-_items.size()):
			if remaining_amount > 0:
