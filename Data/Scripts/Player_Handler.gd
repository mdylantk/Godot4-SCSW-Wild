...
#todo, add a few runs for on_finished and on_cancaled or something
#this logic might be in the run() callable so it may not be needed
#region 
var active_events = {}
...
func handle_event(event):
	...
	#if event["status"] > 0:
		#run it if not finished
		#event["run"].call(event)
	#	pass
	#if event["status"] > 0 and not active_events.has(event["type"]):
		#add it to queue if not finish and not in queue
	#	active_events[event["type"]] = event
	#elif event["status"] <= 0 and active_events.has(event["type"]):
	#	#remove it from queue if finished or cancled if it is in queue
	#	active_events.erase(event["type"])
	#print_debug(event)
func event_process():
